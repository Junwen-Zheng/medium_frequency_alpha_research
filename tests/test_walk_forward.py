...
from src.walk_forward import make_walk_forward_folds, run_walk_forward_validation, run_feature_family_walk_forward, summarize_feature_family_walk_forward_results
...
def test_feature_family_walk_forward_outputs_family_metrics():
    frame = _toy_frame()
    ...
    metrics = run_feature_family_walk_forward(
        frame=frame,
        target="target_rel_fwd_10d",
        family_columns={
            "toy_family": ["feature"],
            "toy_family_copy": ["feature"],
        },
        summarize_predictions=summarize_predictions,
        validation_start="2020-07-01",
        test_start="2021-01-01",
    ...
    assert not metrics.empty
    assert set(metrics["family"]) == {"toy_family", "toy_family_copy"}
    assert np.isfinite(metrics["test_mean_rank_ic"]).all()
    assert metrics["n_features"].min() == 1


def test_feature_family_walk_forward_summary_aggregates_by_family():
    metrics = pd.DataFrame(
        [
            {"fold_id": 1, "family": "raw", "n_features": 2, "validation_mean_rank_ic": 0.01, "test_mean_rank_ic": 0.02, "test_ir": 1.0},
            {"fold_id": 2, "family": "raw", "n_features": 2, "validation_mean_rank_ic": 0.02, "test_mean_rank_ic": -0.01, "test_ir": -0.5},
            {"fold_id": 1, "family": "vol_adj", "n_features": 3, "validation_mean_rank_ic": 0.03, "test_mean_rank_ic": 0.04, "test_ir": 2.0},
            {"fold_id": 2, "family": "vol_adj", "n_features": 3, "validation_mean_rank_ic": 0.01, "test_mean_rank_ic": 0.03, "test_ir": 1.5},
        ]
    )

    summary = summarize_feature_family_walk_forward_results(metrics)

    assert set(summary["family"]) == {"raw", "vol_adj"}
    vol_adj = summary[summary["family"] == "vol_adj"].iloc[0]
    assert vol_adj["n_folds"] == 2
    assert vol_adj["positive_test_folds"] == 2
    assert vol_adj["positive_test_fold_rate"] == 1.0
    assert np.isfinite(vol_adj["mean_test_rank_ic"])
